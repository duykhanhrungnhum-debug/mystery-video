...
AI_WORKER_REVISION = "09b590f103e0cb31b3242fb0394e4aa3a32244d3"
CHECKPOINT_REVISION = "hb-stage-v3-mastery-sync"
...
def main() -> None:
    ...
    ap.add_argument("--output",default="bot2-gpu-submission.json")
    args=ap.parse_args()
    ...
    source_slug=source_ref.split("/",1)[1]
    gpu_slug=("hidden-beyond-bot2-ai-"+video_id.lower().replace("_","-"))[:48]
    gpu_ref=f"{os.environ.get('KAGGLE_USERNAME','duykhanhta')}/{gpu_slug}"
    ...
    sub=client.submit_script(
        slug=gpu_slug,
        title="Hidden Beyond Bot2 AI "+video_id,
        source=worker,
        enable_gpu=True,
